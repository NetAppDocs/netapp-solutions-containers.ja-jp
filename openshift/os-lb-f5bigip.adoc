...
. BIG-IPシステムが命令型コマンドではなくJSON形式の構成を受け入れるようにするには、F5 Application Services 3拡張機能をインストールしてください。link:https://github.com/F5Networks/f5-appsvcs-extension/releases["F5 AS3 GitHubリポジトリ"^]にアクセスし、最新のRPMファイルをダウンロードしてください。
