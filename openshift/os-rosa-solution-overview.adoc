...
* link:https://portal.aws.amazon.com/billing/signup["AWSアカウント"^]
* link:https://console.redhat.com/["Red Hatアカウント"^]
* IAMユーザーlink:https://www.rosaworkshop.io/rosa/1-account_setup/["適切な権限を持つ"^]ROSAクラスターの作成とアクセス
...
* Helm 3 link:https://docs.aws.amazon.com/eks/latest/userguide/helm.html["ドキュメント"^]
...
node-prepフラグを使用して、図に示すように Trident をインストールします。インストールコマンドを実行する前に、インストーラーパッケージをダウンロードしておく必要があります。link:https://docs.netapp.com/us-en/trident/trident-install/kubernetes-deploy-tridentctl.html#step-1-download-the-trident-installer-package["Trident インストーラーパッケージのダウンロード"^]を参照してください。
...
Trident バックエンド設定は、Trident がストレージシステム（この場合は FSx for ONTAP）と通信する方法を定義します。バックエンドを作成するために、接続先のストレージ仮想マシンの認証情報に加え、クラスタ管理と NFS データインターフェースの情報を提供します。link:https://docs.netapp.com/us-en/trident/trident-use/ontap-nas.html["ontap-nas ドライバー"^]を使用して、FSx ファイルシステムにストレージボリュームをプロビジョニングします。
...
----
以下のコマンドを実行すると、PVCを作成し、作成されたことを確認できます。
...
image::redhat-openshift-container-rosa-017.png[PostgreSQLのテーブル、スキーマ、行]
