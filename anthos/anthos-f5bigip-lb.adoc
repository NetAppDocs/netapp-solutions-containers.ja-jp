...
このソリューションは、VMware vSphere にデプロイされた仮想アプライアンスを利用します。F5 Big-IP 仮想アプライアンスのネットワーク構成は、ネットワーク環境に応じて、2アーム構成または3アーム構成に設定できます。本書におけるデプロイは、2アーム構成に基づいています。Anthos で使用するための仮想アプライアンスの設定に関する詳細は、link:https://cloud.google.com/solutions/partners/installing-f5-big-ip-adc-for-gke-on-prem["GKE On-Prem向けF5 BIG-IP ADCインストールガイド"^] を参照してください。
...
. 仮想アプリケーション Open Virtual Appliance (OVA) ファイルをlink:https://downloads.f5.com/esd/serveDownload.jsp?path=/big-ip/big-ip_v15.x/15.0.1/english/virtual-edition/&sw=BIG-IP&pro=big-ip_v15.x&ver=15.0.1&container=Virtual-Edition&file=BIGIP-15.0.1-0.0.11.ALL-vmware.ova["F5ダウンロードポータル"^]からダウンロードします。
+
