...
---
= NetAppを使用した Red Hat OpenShift に関するビデオを見る
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
ディザスタリカバリ、VM 保護、ストレージ管理、 Tridentと Ansible を使用した自動化などの主要な機能を紹介する一連のビデオを通じて、Red Hat OpenShift とNetAppの統合について詳しく説明します。
...
.ワークロードの移行 - Red Hat OpenShift とNetApp
...
**GitHub のサンプル Ansible コード**link:https://github.com/NetApp/trident-install["Ansible を使用してNetApp Trident、StorageClasses、および Backend をインストールするために使用するプレイブックは、github にあります。"]
