...
[%collapsible]
====
この例では、 `postgres`名前空間にインストールされているサンプル PostgreSQL アプリケーションのデータ保護について説明します。インストールの詳細については、link:deploy-vks-workloads.html["NetApp ストレージを使用して VKS ワークロードをデプロイする"]を参照してください。
