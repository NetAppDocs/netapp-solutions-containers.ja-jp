...
NOTE: この時点では、Trident のポッドが作成されない可能性があります。ReplicaSet オブジェクトを確認すると、以下に示すように PodSecurity の違反に関するエラーが表示される場合があります。このエラーは、 `trident` インストール時に作成された名前空間が Kubernetes の制限付き Pod セキュリティ標準（PSS）を適用しているために発生します。Trident Operator はホストストレージを管理するために高いシステム権限を必要とするため、制限付きプロファイルの厳格な制約下では実行できません。これを修正するには、 `trident` 名前空間に特権プロファイルを付与して、ReplicaSet コントローラーがポッドを正常に起動できるようにしてください。
...
----
組み込みの Kubernetes Pod セキュリティ アドミッション ラベルを `trident`名前空間に適用して、制限付きプロファイル制約から除外します：
